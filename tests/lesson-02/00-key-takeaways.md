...
## Tong hop kien thuc ve Javascript
  **1. khai bao bien**
   ...
   **2. khai bao hang**
        const [ten_hang]="gia tri"


  **3.kieu du lieu number**
        + so nguyen 
          const age =25;
        + so thuc
          const price=2000;
        + so vo han 
        const indinify=Indinify;
        + khong phai la so 
        const notNumber=NaN;


  **4. chuoi ki tu string**
      constr name ="nguyen truong"


  **5. boolean ( gia tri ve logic)**
       constr isActive=true;
       constr isActive=false;


  **6. Toan tu so sanh** : dung de so sanh 2 toan hang
       so sanh bang : == va ===
       so sanh khong bang : !=
       so sanh lon hon, nho hon : >,<,>=,<=

  **7. Toan tu logic** : AND , OR
      AND ( &&): tra ve TRUE neu ca 2 dieu kien cung dung
...
##  Tong hop kien thuc ve GIT
 ...
 - **git commit -m "message"**: Lưu lại những gì đã ở staging area thành 1 commit
 - **git push origin main** : cau lenh push code len git 